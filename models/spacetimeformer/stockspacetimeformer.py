...
class StockSpacetimeformer(nn.Module):
    def __init__(self, config):
        ...
        max_seq_len: int = config.seq_len + 1  # +1 for the target token
        attn_factor: int = 5
        d_model: int = 200
        ...
        attn_factor: int = config.factor
        d_model: int = config.d_model
        n_heads: int = config.n_heads
        e_layers: int = config.e_layers
        d_layers: int = config.d_layers
        d_ff: int = config.d_ff
        dropout_emb: float = config.dropout_emb
    ...
    def forward(
        ...
        zeros = torch.zeros([enc_y.shape[0], 1, enc_y.shape[2]]).to(enc_y)
        enc_y = torch.cat([enc_y, zeros], 1)
        enc_x = torch.cat([enc_x, dec_x], 1)

        # enc_self_mask = QuestionMask(
        #     enc_y.shape[0], enc_y.shape[1], device=enc_y.device
        # )

        # embed context sequence
        enc_vt_emb, enc_s_emb, enc_var_idxs, enc_mask_seq = self.enc_embedding(
            y=enc_y, x=enc_x  # switched
        )

        # Filter out enc_vt_emb
        ...
        forecast_out = self.forecaster(enc_out[:, -1:, :])
