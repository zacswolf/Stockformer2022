...
class Informer(nn.Module):
    def __init__(self, config):
        ...
        # Encoding
        self.enc_embedding = DataEmbedding(config.enc_in, config.d_model, config.embed, config.freq, config.dropout)
        ...
        self.encoder = Encoder(
            [
                EncoderLayer(
                    AttentionLayer(Attn(False, config.factor, attention_dropout=config.dropout, output_attention=config.output_attention), 
                                config.d_model, config.n_heads, mix=False),
                    config.d_model,
                    config.d_ff,
        ...
        self.decoder = Decoder(
            [
                DecoderLayer(
                    AttentionLayer(Attn(True, config.factor, attention_dropout=config.dropout, output_attention=False), 
                                config.d_model, config.n_heads, mix=config.mix),
                    AttentionLayer(FullAttention(False, config.factor, attention_dropout=config.dropout, output_attention=False), 
                                config.d_model, config.n_heads, mix=False),
                    config.d_model,
                    config.d_ff,
                    dropout=config.dropout,
...
class InformerStack(nn.Module):
    def __init__(self, config):
        ...
        inp_lens = list(range(len(config.e_layers))) # [0,1,2,...] you can customize here
        encoders = [
            Encoder(
                [
                    EncoderLayer(
